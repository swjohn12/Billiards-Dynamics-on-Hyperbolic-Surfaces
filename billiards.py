...
class Billiards:
    def __init__(self,
        ...
        self.count = 0 # For counting in distance_difference
    ...
    def step(self, dt):
        ...
        if abs(self.time_elapsed - 1) < 0.01 and self.count<1:
            location = [self.state[0], self.state[1]]
            #sqrt((self.state[0]-(1.0/sqrt(2)))**2+(self.state[1]-(1-1.0/sqrt(2)))**2)
            print("location at t=1", dt, location)
            self.count += 1
...
theta2 = 2*pi*random()
ee = 0.2
initial_state = array([(1-ee)*cos(pi/2), (1-ee)*sin(pi/2), -0.3*cos(pi/2+pi/30), -0.3*sin(pi/2+pi/30)], float)
u = Symbol('u')
v = Symbol('v')
g = Matrix([[4/((1-(u**2+v**2))**2), 0], [0, 4/((1-(u**2+v**2))**2)]])
#g = Matrix([[1, 0], [0, 1]])
#g = Matrix([[4/((1-(u**3+v**3))**2), v], [u, 4/((1-(u**3+v**3))**2)]])
#g = Matrix([[1-v**2, 1-u**2], [1-u**2, 1-v**2]])
billiards = Billiards(initial_state, g)
dt = 1./128 # 300 fps
...
def init():
    ...
    return line, time_text, energy_text, init_position, init_velocity
...
def animate(i):
    ...
    init_position.set_text('init_position (deg) = %.1f' % billiards.init_theta_x)
    init_velocity.set_text('init_velocity (deg) = %.1f' % billiards.init_theta_v)
    return line, time_text, energy_text, init_position, init_velocity
...
ani = FuncAnimation(fig, animate, frames=10000, interval = interval, blit=False, init_func=init)
...
#ani.save('flat,dense,pi.3.23231.mp4', fps = 50, extra_args=['-vcodec', 'libx264'])
