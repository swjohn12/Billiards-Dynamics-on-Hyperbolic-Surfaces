...
class Billiards:
    ...
    def step(self, dt):
        ...
        if abs(self.time_elapsed - 1.0) < dt and self.count<1:
            ...
            self.count += 1 """distance is only measured once"""
        ...
        if self.state[0]**2+self.state[1]**2 > (1-e)**2:
            ...
            while (self.state[0]**2+self.state[1]**2 > (1-e)**2): """until the particle leaves the boundary"""
                self.state = geodesic_RK4(self.state, self.dstate_dt, 0, dt)
                self.trajectory.append([self.state[0], self.state[1]])
                self.time_elapsed += dt
...
circle = plt.Circle((0, 0), 1, fill=False) # boundary disk
